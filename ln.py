...
dataset.head()    #DataFrame.head(n=5)[source]=> Return the first n rows(int, default 5).

# Show dataset overview, should return 1000 for each alphabet (excluding y and z)
dataset['class'].value_counts()  #Return a Series containing counts of unique rows in the DataFrame.
...
scaler = StandardScaler().fit(X_train)  #Compute the mean and std to be used for later scaling[fit(x)]  #StandardScaler removes the mean and scales each feature/variable to unit variance. This operation is performed feature-wise in an independent way. StandardScaler can be influenced by outliers

X_train = scaler.transform(X_train)  #transform(X) => Perform standardization by centering and scaling.
X_test = scaler.transform(X_test)

# inverse_transform(X, copy=None) => Scale back the data to the original representation.
